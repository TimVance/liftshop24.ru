...
if (! empty($result["rows"]))
{
	foreach ($result["rows"] as $row) //вывод полей из конструктора форм
	{
		...
		switch ($row["type"])
		{
			...
			case 'text':
				echo '
				<input type="text" name="p'.$row["id"].'" value="" placeholder="'.$row["name"].($row["required"] ? '*' : '').'">';
				break;

			case "email":
				echo '
				<input type="email" name="p'.$row["id"].'" value="" placeholder="'.$row["name"].($row["required"] ? '*' : '').'">';
				break;

			case "phone":
				echo '
				<input type="tel" name="p'.$row["id"].'" value="" placeholder="'.$row["name"].($row["required"] ? '*' : '').'">';
				break;

			case 'textarea':
				echo '
				<textarea minlength="30" name="p'.$row["id"].'" placeholder="Подробно опишите свою проблему (не меньше 30-ти символов)"></textarea>';
				break;
			...
			case 'checkbox':
				echo '<input name="p'.$row["id"].'" id="feedback_p'.$row["id"].'" value="1" type="checkbox" ><label for="feedback_p'.$row["id"].'">'.$row["name"].($row["required"] ? '' : '').'</label>';
				break;
...
echo '<div class="privacy-info">Отправляя форму, я даю согласие на <a href="'.BASE_PATH.'privacy/">обработку персональных данных</a></div>';
