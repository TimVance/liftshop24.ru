...
if ($result["row"]["price_arr"] && $result["row"]["price_arr"][0]["price"] != 0)
{
	foreach ($result["row"]["price_arr"] as $price)
	{
		...
		echo '<div class="js_shop_param_price shop_param_price shop-item-price"'.$param_code.'>';
			echo '<span class="pricerow"><span class="js_shop_price" summ="'.$price["price_no_format"].'" format_price_1="'.$this->diafan->configmodules("format_price_1", "shop").'" format_price_2="'.$this->diafan->configmodules("format_price_2", "shop").'" format_price_3="'.$this->diafan->configmodules("format_price_3", "shop").'">'.$price["price"].'</span> '.$result["result"]["currency"];
...
else echo '<div class="pricerow_zapros">По запросу</div>';
...
echo '<div class="js_shop_buy shop_buy to-cart-row flexBetween">';
	if (empty($result["row"]['is_file']) && empty($hide_submit))
	{
		echo '<input type="number" value="1" name="count" class="numberrows" pattern="[0-9]+([\.|,][0-9]+)?" step="any">';
	}
	if(empty($hide_submit))
	{
		echo '<input type="button" class="button solid inCartrow" value="'.$this->diafan->_('В корзину', false).'" action="buy">';
	}
echo '</div>';

if(empty($hide_submit) && ! empty($result["result"]["one_click"]))
{
	echo '<span class="js_shop_one_click shop_one_click"><input class="oneclickbuttonrow" type="button" value="'.$this->diafan->_('Заказать в один клик', false).'" action="one_click"></span>';
}
echo '</form>';
