...
if(empty($result['rows'])) return false;
		...
		{
			echo '<div class="js_shop shop-banner">';

				echo '<div class="flexBetween banner-inside">';

					echo '<div>';
					//вывод названия и ссылки на товар	
					echo '<a title="'.$row["name"].'" href="'.BASE_PATH_HREF.$row["link"].'" class="shop-item-title">'.$row["name"].'</a>';	
					echo $this->get('buy_form_catbanner', 'shop', array("row" => $row, "result" => $result));
					echo '</div>';
