...
class Shop_model extends Model
{
	 ...
	 * @return array
	 */
	public function show_block($count, $site_ids, $ids, $cat_ids, $brand_ids, $sort, $images, $images_variation, $param, $hits_only, $action_only, $new_only, $discount_only, $tag)
	{
		$time = mktime(23, 59, 0, date("m"), date("d"), date("Y"));
	...
	public function show_search($site_ids, $cat_ids, $ajax)
	{
		...
		if (! empty($result["article"]))
		{
			$result["name"]["value"] = ! empty($_REQUEST["name"]) ? trim(htmlspecialchars(stripslashes($_REQUEST["name"]))) : '';
		}
