			...
			while(1==1)
			{
				...
				foreach ($rows as $row)
				{
					...
					if (empty($yandex["typePrefix"]) || empty($yandex["vendor"]) || empty($yandex["vendorCode"]) || empty($yandex["model"]))
					{
						...
						if (empty($pictures)) $pictures[] = 'https://liftshop24.ru/custom/custom31_07_2018_15_15/img/orig.webp';
						...
						if(!empty($row["text".$this->diafan->_languages->site]))
						{
							$text .= '
							...
							$text .= '
							<description>'.$this->prepare($row["name".$this->diafan->_languages->site]).'</description>';
						}
						$text .= '
