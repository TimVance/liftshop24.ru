...
class Postman_admin extends Frame_admin
{
	...
	public $variables = array (
		'main' => array (
			...
			'recipient' => array(
				...
				'help' => 'Для почтового уведомления необходимо указать адрес или адреса через пробел получателя. Для СМС уведомления - адрес получателя.',
			),
			'subject' => array(
				'type' => 'text',
				'name' => 'Тема уведомления',
				'help' => 'Содержание заголовка почтового уведомления. Для СМС уведомления не указывается.',
			),
			'body' => array(
				'type' => 'function',
			...
			'from' => array(
				...
				'help' => 'Указывается для почтового уведомления адрес, от имени которого будет направлено письмо. Для СМС уведомления не указывается.',
			),
			'error' => array(
				'type' => 'text',
	...
	public $variables_list = array (
		...
		'subject' => array(
			...
			'sql' => true,
			'class' => 'text',
			'help' => 'Содержание уведомления для пустой темы или смс уведомления. В иных случаях - тема уведомления.',
			'no_important' => true,
		),
		'from' => array(
			'type' => 'text',
	...
	public function show()
	{
		...
		else
		{
			...
			if(! defined('SMTP_MAIL') || ! SMTP_MAIL)
			{
				$this->important_title =
					'<div class="head-box head-box_warning">
						...
						.$this->diafan->_('Для корректной отправки писем важно, чтобы в настройках сайта был указан тот же почтовый ящик администратора, который является отправителем всех уведомлений в настройках модулей. А также очень важно использовать SMTP-авторизацию исходящей почты.')
						.' '
						.$this->diafan->_("Настроить %sSMTP-авторизацию%s", '<a href="'.BASE_PATH_HREF.'config/" target="_blank">', '</a>')
						.' '
						.'('.$this->diafan->_('параметры настроек необходимо уточнять у администраторов используемого почтового сервера').').'
						.$this->help('Раздел "Настройки сайта", пункт "Использовать SMTP-авторизацию при отправке почты с сайта".')
					.'</div>';
			}
			...
			$statmailnosend = DB::query_result("SELECT COUNT(*) FROM {%s} WHERE ".$typestat." status='0'", $this->diafan->table, 'mail');
			$statsmsnosend = DB::query_result("SELECT COUNT(*) FROM {%s} WHERE ".$typestat." status='0'", $this->diafan->table, 'sms');
			echo $this->diafan->_('Всего <b>писем</b> / <b>смс</b> ожидает отправки: <b>%s</b> / <b>%s</b>', $statmailnosend, $statsmsnosend);

			$statmailsenderror = DB::query_result("SELECT COUNT(*) FROM {%s} WHERE ".$typestat." status='2'", $this->diafan->table, 'mail');
			$statsmssenderror = DB::query_result("SELECT COUNT(*) FROM {%s} WHERE ".$typestat." status='2'", $this->diafan->table, 'sms');
			echo ($statmailsenderror > 0 || $statsmssenderror > 0 ? $this->diafan->_(', ошибки отправлений: <b>%s</b> / <b>%s</b>', $statmailsenderror, $statsmssenderror) : '');
			...
			if(! defined('SMTP_MAIL') || ! SMTP_MAIL)
			{
				$this->important_title =
					'<div class="head-box head-box_warning">
						...
						.' '
						.'('.$this->diafan->_('параметры настроек необходимо уточнять у администраторов используемого почтового сервера').').'
						.$this->help('Раздел "Настройки сата", пункт "Использовать SMTP-авторизацию при отправке почты с сайта".')
					.'</div>';
			}
	...
	public function validate_variable_send()
	{
		if(! empty($_POST["send"]))
		{
			...
			if(! $error && ! empty($_POST["type"]) && $_POST["type"] == 'sms' && (! defined('SMS') || ! SMS || ! defined('SMS_KEY') || ! SMS_KEY || ! defined('SMS_ID') || ! SMS_ID || ! defined('SMS_SIGNATURE') || ! SMS_SIGNATURE))
			{
