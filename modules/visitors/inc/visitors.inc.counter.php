...
class Visitors_inc_counter extends Diafan
{
	...
	/**
	 * @var boolean маркер инициализации валидации пользовательского агента
	 */
	private $is_validator = false;
	...
	public function init($only_valid = false)
	{
		...
		$params["hostname"] = getenv("REMOTE_ADDR");
		...
		$result = $this->diafan->fast_request(BASE_PATH.self::URL, $params, "POST");

		// блокировка ботов
		if(! isset($params["VALID_NAME"]) && ! isset($params["VALID_VALUE"]) && empty($params["valid"]))
		{
	...
	public function set($params)
	{
		...
		$SESS = array(
			...
			"valid_complete" => isset($params["VALID_COMPLETE"]),
		);
	...
	private function validator()
	{
		...
		if(! isset($_SESSION["visitors"]["valid"]) || (! isset($_SESSION["visitors"]["VALID_COMPLETE"]) && $this->diafan->configmodules('counter_defer', 'visitors')))
		{// валидация пользовательского агента
...
<body>
	<script language="javascript" type="text/javascript">
		...
		else
		{
			...
			if(isset($_SESSION["visitors"]["VALID_NAME"]) && isset($_SESSION["visitors"]["VALID_VALUE"]))
			{
				$expires = 60*60*24*1; // 1 сутки
				$path = '/';
				$domain = $this->diafan->_session->HTTP_HOST();
	...
	/**
	 * Устанавливате маркер завершения процесса инициализации валидации пользовательского агента
	 *
	 * @return void
	 */
	public function validator_complete()
	{
		if($this->is_validator)
		{
			$_SESSION["visitors"]["VALID_COMPLETE"] = true;
		}
	}
